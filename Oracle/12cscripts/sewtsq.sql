...
col username Head "Username" form a25
...
col sql_text form a28 trunc

select 	distinct sid, username, event, p1, p2, p3, sql_id, sql_text
from 	(
	select	/*+leading(sw)*/
		s.sid, s.username, s.event, s.p1, s.p2, s.p3, s.sql_id, SQL_CHILD_NUMBER, sqwn.sql_text sql_text
	from    v$session 		s,
