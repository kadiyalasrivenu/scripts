...
col HOT_USED_MB head "Hot Used|(In MB)" form 999,999,999
col COLD_USED_MB head "Cold Used|(In MB)" form 999,999,999
...
select	adg.GROUP_NUMBER, adg.NAME groupname, 
	adg.TOTAL_MB, adg.FREE_MB, 
	adg.HOT_USED_MB, adg.COLD_USED_MB,
	adg.REQUIRED_MIRROR_FREE_MB, adg.USABLE_FILE_MB
from 	v$asm_diskgroup adg
where 	adg.name = upper(trim('&1'))
order 	by 1,2
/
...
col DISK_NUMBER head "Disk|No" form 999
col diskname head "Disk name" form a20
col incarnation head "Incar|nation" form 9999999999
col library head "Library" form a10
col failgroup head "Fail|Group" form a10
col udid head "UDID" form a10
col product head "Product" form a10
