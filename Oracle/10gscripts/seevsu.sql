...
-- Modified by srivenu kadiyala for session event waits
--              
...
BREAK ON lhp_event SKIP 1

DEF _IF_ORA_10_OR_HIGHER="--"

PROMPT
PROMPT -- LatchProf 1.06 by Tanel Poder ( http://www.tanelpoder.com )
...
WITH 
    ...
    samples AS (
		SELECT /*+ ORDERED USE_NL(l.GV$SESSION.s)*/
			&_lhp_what
		  &_IF_ORA_10_OR_HIGHER , COUNT(DISTINCT seq#)		dist_samples
		  , COUNT(*) 					total_samples
		...
		WHERE
			l.sid LIKE '&_lhp_sid'
		AND (LOWER(l.event) LIKE LOWER('%&_lhp_event%'))
		AND state='WAITING'
		GROUP BY
			&_lhp_what
		ORDER BY
