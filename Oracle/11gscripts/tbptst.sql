...
select 	owner, table_name,
	...
	blocks, empty_blocks, 
	GLOBAL_STATS, USER_STATS, SAMPLE_SIZE
from 	dba_tables 
where 	OWNER = upper('&1')
...
select	rownum ser, v.* 
from	(
	select 	dts.SUBPARTITION_POSITION, dts.partition_name, dts.subpartition_name, 
		to_char(dts.last_analyzed,'dd-mon hh24:mi') last_analyzed,
		dts.avg_space, dts.avg_row_len, dts.num_rows,
		dts.blocks, dts.empty_blocks, 
		dts.GLOBAL_STATS, dts.USER_STATS, dts.SAMPLE_SIZE
	...
	order 	by dtp.partition_position, dts.subpartition_position
	) v
order 	by rownum
/
